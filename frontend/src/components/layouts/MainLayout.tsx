...
import {getPublicKey, login, logout, wallet} from "~/utils/near";
    ...
    return (
        <div>
            <Layout className={"bg-color sm:px-0 md:px-3 xl:px-5 lg:px-5"} style={{minHeight: "100vh"}}>
                <Header
                    className={"padding-0-15 flex flex-row text-white justify-center items-center bg-color border-b border-gray-900"}>
                    <div style={{width: 150}}>
                        <Link to={"/"} className={"flex justify-start items-center"}>
                            <img src={logo} style={{width: 40, height: 40, marginRight: 15}} alt=""/>
                            <span className={"text-2xl font-extrabold sm:hidden xs:hidden"}>LINE</span>
                        </Link>
                    </div>
                    ...
                    <div className={"xs:flex-1 sm:flex-1 md:flex-1 flex justify-end items-center"} style={{width: 140}}>
                        <LoginModal/>
                        {
                            wallet.isSignedIn() ?
                                <>
                                    <Button onClick={() => logout()} shape="round" className={"ant-button-hover-white"}>
